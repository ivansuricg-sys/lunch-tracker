...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (authed) loadResumen();
  }, [authed, loadResumen]);

  useEffect(() => {
    if (authed) loadPersonas();
  }, [authed, loadPersonas]);

  // Build "por día" view from resumen data
  const byDia = useMemo(() => {
    const map = new Map<string, string[]>();
    for (const row of resumen) {
      for (const d of row.dias) {
        if (!map.has(d.fecha)) map.set(d.fecha, []);
        map.get(d.fecha)!.push(row.nombre);
      }
    }
      ...
      <section className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-gray-700">
            Resumen — {getMonthName(mes)} {anio}
          </h2>
          {!loadingResumen && (
            <span className="text-xs text-gray-400">
              {resumen.reduce((s, r) => s + r.cantidad, 0)} registros
            </span>
          )}
        </div>
          ...
          <div>
            {resumen.map((row) => (
              <div key={row.nombre} className="border-b border-gray-50 last:border-0">
                <button
                  onClick={() =>
                    setExpandedPersona(expandedPersona === row.nombre ? null : row.nombre)
                  }
                ...
                {expandedPersona === row.nombre && (
                  <div className="px-4 pb-3 bg-gray-50">
                    <div className="space-y-1.5">
                      {row.dias.map((d) => (
                        <div key={d.fecha} className="flex justify-between text-sm">
      ...
      {byDia.length > 0 && (
        <section className="bg-white rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {byDia.map(({ fecha, personas: ps }) => (
              <div key={fecha} className="px-4 py-3">
                <div className="flex items-start gap-3">
                  <span className="text-sm font-medium text-gray-500 min-w-[88px]">{fecha}</span>
                  <span className="text-sm text-gray-700">{ps.join(', ')}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div key={p.id} className="flex items-center justify-between px-4 py-3">
              <span
                className={`font-medium ${
                  p.activo ? 'text-gray-800' : 'text-gray-400 line-through'
                }`}
              >
                {p.nombre}
              </span>
